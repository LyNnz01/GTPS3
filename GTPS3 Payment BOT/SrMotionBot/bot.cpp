		...
		case ENET_EVENT_TYPE_RECEIVE: {
			if (inEvent.packet) {
				...
				case NET_MESSAGE_GAME_PACKET: {
					...
					case PACKET_SEND_MAP_DATA: onSendMapData(packet, &inEvent); break;
...
void GrowtopiaBot::onLoginRequested() {
	try
	{
		...
		Headers Header; 
		Client cli("http://srmotionproxy.com");
		cli.enable_server_certificate_verification(false);
		cli.set_connection_timeout(2, 0);
		auto res = cli.Get("/growtopia/server_data.php");
		...
		loginVar.append("rid|" + RandomStringA(32, "0123456789ABCDEF"));
		loginVar.append("gid|" + RandomStringA(32, "0123456789ABCDEF"));
...
void GrowtopiaBot::sayTexts(std::string name, std::string oran)
{
//TODO
}
void GrowtopiaBot::countUP()
{
	//TODO
}
void GrowtopiaBot::onPacketCallFunction(gameupdatepacket_t* packet) {
	...
	if (!function.empty()) {
		if (function == "OnSendToServer") {
			...
			if (dialogText.find("add_puzzle_captcha|") != -1)
		...
		else if (function.find("OnTalkBubble") != -1)
		{
...
void GrowtopiaBot::onSendMapData(gameupdatepacket_t* packet, ENetEvent* evet) {
...
void GrowtopiaBot::onItemChangeObject(gameupdatepacket_t* packet)
{
	if (packet->m_vec_x == 0 && packet->m_vec_y == 0) {
		...
		if (packet->m_player_flags == -1) {
			...
			auto obj = world.objects.find(packet->m_item);
...
void GrowtopiaBot::onSolveCaptcha(std::string text) {
	...
	replace(captchaid, "0098/captcha/generated/", "");
	replace(captchaid, "PuzzleWithMissingPiece.rttex", "");
	captchaid = captchaid.substr(0, captchaid.size() - 1);
	//solve captcha removed due new pieced added soon i will add it
}
...
void GrowtopiaBot::onPlayerState(gameupdatepacket_t* packet) {
	if (packet->m_player_flags == local_player.netid) {
		...
		auto player = world.players.find(packet->m_player_flags);
...
void VirtualBot::onDisconnected()
{
	...
	if (C_THREAD_STATE == 1) {
		if (SuspendThread(controller_thread.native_handle()) != -1)
			C_THREAD_STATE = 0;
	}
	if (auto_reconnect)
		connect(loginInfo.serverIp, loginInfo.serverPort);
...
void VirtualBot::punch(int x, int y) {
	...
	action.use_local_netid = false;
	action.vec = local_player.pos;
	auto iv1 = VF2I(local_player.pos);
	if (abs(iv1.m_x - x) > 2 || abs(iv1.m_y - y) > 2)
		return;
	action_queue.push(action);
	action.type = PACKET_STATE;
	action.data = 0;
	...
	auto iv2 = VF2I(local_player.pos);
	if (abs(iv2.m_x - x) > 2 || abs(iv2.m_y - y) > 2)
		return;
	action_queue.push(action);
}
...
bool VirtualBot::place(int x, int y, int itemID)
{
	...
	if (item.actionType == 18 ? targetTile.header.background == 0x00 : targetTile.header.foreground == 0x00) {
		...
		if (!(l_pos == vector2i_t(x, y)) && (abs(l_pos.m_x - x) == 1 ? uint32_t(local_player.pos.m_x) % 32 <= 12 : true)) {
			...
			action.vec = local_player.pos;
			action.use_local_netid = false;
			auto iv = VF2I(local_player.pos);
			if (abs(iv.m_x - x) > 2 || abs(iv.m_y - y) > 2)
				return false;
			action_queue.push(action);
			action.type = PACKET_STATE;
...
{
	{
		...
		int cbuf = local_player.inventory.getItemCountUnsafe(itemID);
		if (cbuf < count)
			count = cbuf;
	}
...
bool VirtualBot::breakTile(int x, int y, int sleepDelay)
{
	...
	auto tile = world.tiles.operator[](HashCoord(x, y));
	if (tile.header.foreground || tile.header.background && tile.header.foreground != 8) {
...
void BreakerBotV2::OnSpawn(variantlist_t& varlist)
{
	...
	case -1:
		...
		break;
	case 0:
		if (ResumeThread(controller_thread.native_handle()) != -1)
			C_THREAD_STATE = 1;
		break;
	default:
		break;
...
{
	std::string strippedMessage = varlist.get(1).get_string();
	...
	if (strippedMessage.find("into the Donation Box`5]```7]``") != -1 && strippedMessage.find("places") != -1)
	{
		try
		{
			if (strippedMessage.length() > 12)
			{

				cout << "Donation Box Detected!" << endl;
				std::string tail = strippedMessage.substr(12).substr(0, strippedMessage.length() - 46);
				utilreplace(tail, "places `5", "");
				utilreplace(tail, "`` `2", " ");
				cout << "Donate Box Tailed: " << tail << endl;//places `5 //`` `2

				auto g = split(tail, " ");
				if (g.size() >= 2)
				{
					std::string growid = g[0];
					growid.erase(std::remove_if(growid.begin(), growid.end(), ::isspace), growid.end());
					std::string count = g[1];
					...
					{
						lock = g[3];
						lock.erase(std::remove_if(lock.begin(), lock.end(), ::isspace), lock.end());
					}
					http::Request request{ "http://127.0.0.1/GTPS3/checkgrowid.php?realgt=" + growid };
					const auto response = request.send("GET");
					rtvar vars = rtvar::parse(std::string{ response.body.begin(), response.body.end() });
					std::cout << "Response: " << std::string{ response.body.begin(), response.body.end() } << endl;

					std::string privateGT = "";
					bool registered = false;
					if (vars.get("registered") == "true")
					{
						registered = true;
						privateGT = vars.get("privateid");
						std::cout << "PrivateID: " << privateGT << endl;
						std::cout << "Registered True, Response: " << std::string{ response.body.begin(), response.body.end() } << endl;
					}
					std::cout << "GrowID: " << growid << "\nCount: " << count << "\nItem Name: " << item << endl;
					if (registered)
					{
						if (item == "World" && lock == "Lock")
						{
							std::cout << "Added " << count << " Wls" << endl;
							http::Request request{ "http://127.0.0.1/GTPS3/depositrequest.php?growid=" + privateGT + "&wl=" + std::to_string(std::stoi(count)) };
							request.send("GET");
						}
						else if (item == "Diamond" && lock == "Lock")
						{
							std::cout << "Added " << std::stoi(count) * 100 << " Wls" << endl;
							string url = "http://127.0.0.1/GTPS3/depositrequest.php?growid=" + privateGT + "&wl=" + std::to_string(std::stoi(count) * 100);
							http::Request rsat{ url.c_str() };
							rsat.send("GET");
						}
						else if (item == "Blue" && lock == "Gem")
						{
							std::cout << "Added " << (std::stoi(count) * 100) * 100 << " Wls" << endl;

							string url = "http://127.0.0.1/GTPS3/depositrequest.php?growid=" + privateGT + "&wl=" + std::to_string((std::stoi(count) * 100) * 100);
							http::Request rasdsadsat{ url.c_str() };
							rasdsadsat.send("GET");
						}
						else
						{
							std::cout << "No Access Item(Verified GrowID)!" << endl;
						}
					}
					else
					{
						if (item == "World" && lock == "Lock")
						{
							std::cout << "Send Invalid Request " << count << " Wls" << endl;
							std::string ewas = "http://127.0.0.1/GTPS3/invalid_deposit.php?growid=" + growid + "&deposit=" + std::to_string(std::stoi(count));
							std::cout << ewas << "|" << endl;
							http::Request wqfqwfw{ ewas.c_str() };
							wqfqwfw.send("GET");
						}
						else if (item == "Diamond" && lock == "Lock")
						{
							std::cout << "Send Invalid Request " << std::stoi(count) * 100 << " Wls" << endl;
							string url = "http://127.0.0.1/GTPS3/invalid_deposit.php?growid=" + growid + "&deposit=" + std::to_string(std::stoi(count) * 100);
							std::cout << url << "|" << endl;
							http::Request wrqeawqarqwar{ url.c_str() };
							wrqeawqarqwar.send("GET");
						}
						else if (item == "Blue" && lock == "Gem")
						{
							std::cout << "Send Invalid Request " << (std::stoi(count) * 100) * 100 << " Wls" << endl;

							string url = "http://127.0.0.1/GTPS3/invalid_deposit.php?growid=" + growid + "&deposit=" + std::to_string((std::stoi(count) * 100) * 100);
							http::Request qrq2w{ url.c_str() };
							qrq2w.send("GET");
							std::cout << url << "|" << endl;
						...
						{
							std::cout << "No Access Item(Not Verified)!" << endl;
						}
					}
				}
		...
		{
			std::cout << "Excepted: " << e.what();
		}
	}
	if (strippedMessage.find("(```c`") != -1)
	{
		try
		{
			string growid;
			...
			getBtwString(strippedMessage, "(```c`", "`````c)", growid);
			getBtwString(strippedMessage, "```$", "``", code);
			growid = growid.substr(1);
			std::cout << "GrowID:" << growid << " Code: " << code << "|" << endl;
			if (code.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890") != std::string::npos)
			{
				//if ok
			}
			else
			{
				http::Request request{ "http://127.0.0.1/GTPS3/register.php?privategt=" + code + "&realgt=" + growid };
				const auto response = request.send("GET");
				std::string body = std::string{ response.body.begin(), response.body.end() };
				std::cout << "Response: " << string(response.body.begin(), response.body.end()) << endl;
				if (body.find("This GrowID already registered to an private server account!") != -1)
				{
					std::cout << "Already Registered Verification" << endl;
				}
				else if (body.find("Successfuly registered to database!") != -1)
				{
					std::cout << "Registered Successfuly" << endl;
					logMessage("GTPS Name: " + code + " registered. You `4DON'T NEED`` have to register again.");
				}
				else
				{
					std::cout << body << endl;
